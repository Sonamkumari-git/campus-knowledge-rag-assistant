...
Authentication uses Google's OAuth 2.0 flow rather than a home-grown password form. The application never stores a plaintext password and keeps `GOOGLE_CLIENT_SECRET` server-side. Configure `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, and `GOOGLE_REDIRECT_URI` in the deployment environment. The redirect URI must exactly match the Google Cloud OAuth client configuration; for the Render deployment it is `https://campus-knowledge-ai.onrender.com/api/oauth/callback`.

The header's **Sign in** and **Create account** actions both open Google account selection through the nonce-protected `/api/oauth/google/start` route. Google automatically creates the application user record the first time a Gmail account completes the flow. The callback exchanges the authorization code, stores the Google subject identifier and profile in MongoDB, and creates the application's secure session cookie.
